...
def parse_de_price(value: str) -> float:
    return float(_clean_num_token(value).replace(",-", ".0").replace(",", "."))
...
def parse_dimension_pair(value: str) -> tuple[int, int] | None:
    ...
    if not match:
        return None
    return parse_de_to_mm(match.group(1)), parse_de_to_mm(match.group(2))
...
def order_dimensions(width_mm: int, height_mm: int) -> tuple[int, int]:
    return max(0, width_mm - ORDER_DEDUCTION_MM), max(0, height_mm - ORDER_DEDUCTION_MM)


def compose_description(row: dict) -> str:
    parts = [
        f"RBM {rohbau_text(int(row['width_mm']), int(row['height_mm']))}",
...
def detect_pdf_format(pdf_path: str) -> str:
    ...
    for line in text.splitlines():
        if TABLE_ID_RE.match(line.strip()):
            table_line_hits += 1

    plan_id_hits = len(PLAN_ID_RE.findall(text))
    rbm_hits = len(PLAN_RBM_RE.findall(text))
    has_plan_text = any(x in text for x in ["Drehflügel", "Dreh- Kippflügel", "DIN links", "DIN rechts", "Fenstertür"])

    if table_line_hits >= 3:
        return "table"
    if plan_id_hits >= 8 and rbm_hits >= 5 and has_plan_text:
        return "plan"
    return "unknown"
        ...
        for line in text.splitlines():
            ...
            try:
                results.append({
                    ...
                    "height_mm": int(m.group(2)),
                    "price_garda_82": parse_de_price(num_tokens[7]),
                    "glass_area_index_m2": parse_de_area(num_tokens[13]),
                    "page": page_idx + 1,
                })
            except ValueError:
                continue
...
def lookup_supplier_match(row: dict) -> dict:
    ...
    lookup_width = int(row["order_width_mm"])
    lookup_height = int(row["order_height_mm"])
    ...
    best = min(candidates, key=lambda item: abs(item["width_mm"] - lookup_width) + abs(item["height_mm"] - lookup_height))
...
def enrich_rows_with_supplier_lookup(rows: list[dict]) -> list[dict]:
    ...
    for row in rows:
        ...
        enriched_row["order_width_mm"], enriched_row["order_height_mm"] = order_dimensions(
            int(enriched_row["width_mm"]), int(enriched_row["height_mm"])
        )
...
def _extract_operation_specs(full_text: str) -> dict[int, dict]:
    ...
    for idx, m in enumerate(matches):
        ...
        specs[spec_no] = {
            "spec_no": spec_no,
            "operation_type": m.group(1).replace("Dreh- Kippflügel", "Dreh-Kippflügel").strip(),
...
def _extract_rows_from_plan_pdf(pdf_path: str) -> list[dict]:
    ...
    for m in raw_matches:
        ...
        if raw_id in seen_ids or not re.fullmatch(r"F-\d+(?:\.\d+)+", raw_id):
            continue
        seen_ids.add(raw_id)
        ...
        rows.append({
            ...
            "room_no": "",
            "room_name": "",
            "type": operation_type or type_hint,
...
def _looks_valid(rows: list[dict]) -> bool:
    if len(rows) < 1:
        return False
    plausible = 0
    for r in rows:
        if 300 <= int(r["width_mm"]) <= 6000 and 300 <= int(r["height_mm"]) <= 6000:
            plausible += 1
...
def extract_rows_from_pdf(pdf_bytes: bytes) -> list[dict]:
    ...
    try:
        ...
        if detected == "table":
            attempts = [_extract_rows_from_table_pdf, _extract_rows_from_plan_pdf]
        elif detected == "plan":
            attempts = [_extract_rows_from_plan_pdf, _extract_rows_from_table_pdf]
        else:
            attempts = [_extract_rows_from_table_pdf, _extract_rows_from_plan_pdf]
        ...
        if last_rows and _looks_valid(last_rows):
            return last_rows
    finally:
        Path(tmp_path).unlink(missing_ok=True)

    raise ValueError(
        "Unbekanntes PDF-Format oder keine gültigen Fensterzeilen gefunden. "
...
def populate_report_sheet(wb, rows: list[dict], warnings: list[str], source_pdf: str, house_mode: str, aggregate: bool):
    ...
    row_cursor += 1
    headers = [
        "ID", "Haus", "Geschoss", "Menge", "RBM_Breite_mm", "RBM_Höhe_mm", "Bestell_Breite_mm", "Bestell_Höhe_mm",
        "Beschreibung", "Typ", "Raumnr.", "Raumname", "Lieferantentyp", "Lookup_Breite_mm", "Lookup_Höhe_mm",
    ...
    for item in rows:
        vals = [
            ...
            item.get("order_width_mm"), item.get("order_height_mm"), item["description"], item["type"],
            item.get("room_no", ""), item.get("room_name", ""), item.get("supplier_type"), item.get("lookup_width_mm"),
            item.get("lookup_height_mm"), item.get("supplier_price_eur"), item.get("glass_area_index_m2"),
            item.get("supplier_confidence"), item.get("supplier_match_note")
        ]
