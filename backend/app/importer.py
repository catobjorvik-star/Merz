...
PLAN_SPEC_START_RE = re.compile(r"^(?:UG|EG|OG|DG)?\s*(\d{1,2})\s+\d+\s*Stk\.?$", re.I)
PLAN_OP_RE = re.compile(r"^(Dreh(?:-\s*Kipp)?flügel|Fenstertür|Festverglasung|Ausführung als Haustür)", re.I)
PLAN_DIN_RE = re.compile(r"^DIN\s+(rechts|links)(?:,\s*DIN\s+(rechts|links))?(?:\s*\(!\))?$", re.I)
PLAN_FLOOR_RE = re.compile(r"^(UG|EG|OG|DG)$", re.I)
MM = 1000
...
def is_supported_id(raw_id: str) -> bool:
    rid = normalize_id(raw_id)
    return bool(TABLE_ID_RE.match(rid) or PLAN_ID_RE.match(rid))
...
def _price_candidates_for_group(group: str) -> list[dict]:
    ...
    for page_idx in SUPPLIER_PAGE_GROUPS.get(group, []):
        ...
        for line in text.splitlines():
            ...
            try:
                garda_d = parse_de_price(num_tokens[6])
                garda_82 = parse_de_price(num_tokens[7])
                garda_hf = parse_de_price(num_tokens[8])
            ...
            results.append({
                ...
                "width_mm": width,
                "height_mm": height,
                "price_garda_d": garda_d,
                "price_garda_82": garda_82,
                "price_garda_hf": garda_hf,
...
def _extract_rows_from_plan_pdf(pdf_path: str) -> list[dict]:
    ...
    for m in raw_matches:
        ...
        width_mm = parse_de_to_mm(dim_match.group(1))
        height_mm = parse_de_to_mm(dim_match.group(2))
        ...
        brh_mm = parse_de_to_mm(brh_match.group(1)) if brh_match else None
        ...
        for bl in body_lines:
            ...
            elif PLAN_OP_RE.match(bl):
                type_hint = bl.replace("Dreh- Kippflügel", "Dreh-Kippflügel").strip()
...
def write_to_template(excel_bytes: bytes, rows: list[dict], warnings: list[str], source_pdf_name: str, house_mode: str, aggregate: bool) -> bytes:
    ...
    try:
        ...
        ws.column_dimensions["M"].width = max(ws.column_dimensions["M"].width or 8, 28)
