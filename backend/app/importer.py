...
def parse_plan_dim_to_mm(value: str) -> int:
    """Fensterplan values like 113 / 257 mean centimeters, not meters."""
    s = str(value or "").strip().replace(" ", "").replace("\n", "")
    s = s.replace(",", ".")
    val = float(s)
    if "." in s:
        return int(round(val * 10))   # 113,5 cm -> 1135 mm
    return int(round(val * 10))       # 113 cm -> 1130 mm
...
def _extract_rows_from_plan_pdf(pdf_path: str) -> list[dict]:
    ...
    # merge F-1.7 and F-1.8 into one usable row if F-1.8 lacks RBM block in header text
